...
def generate_reply(tokenizer, model, messages: list[dict], gen_cfg: dict) -> str:
    ...
    # return_dict=True is required explicitly (not just return_tensors="pt"):
    # newer transformers versions return a BatchEncoding here regardless, and
    # passing that positionally into model.generate() as if it were a plain
    # tensor fails (BatchEncoding has no .shape). Requesting the dict and
    # unpacking it via **inputs also gets us attention_mask for free, which
    # the previous plain-tensor call never passed to generate() at all.
